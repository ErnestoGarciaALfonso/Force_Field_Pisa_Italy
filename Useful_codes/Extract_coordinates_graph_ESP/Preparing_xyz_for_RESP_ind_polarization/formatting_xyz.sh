...
INPUT_FILE="coordinates"  # Changed to more common extension
...
ATOM_NUMBER=("1" "8" "6" "8" "6" "1" "6" "1" "1" "16" "7" "1" "6" "8" "6" "1" "1" "1")
...
for ((LINE_NUM=26; LINE_NUM<=TOTAL_LINES; LINE_NUM++)); do
    ...
    printf "%-12s %-12s %-12s %2s %-3s \n" \
           $(echo $LINE) "${ATOM_NUMBER[$CYCLE_POS]}" "${ATOM_TYPES[$CYCLE_POS]}" >> "$OUTPUT_FILE"
done
